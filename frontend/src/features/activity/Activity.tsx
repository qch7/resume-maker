...
export default function Activity() {
  ...
  }, [category, level, keyword, trace, since, until]);
    ...
    <section className="activity-workspace" aria-label="系统日志">
      <header className="activity-heading">
        <div className="activity-heading-copy">
          ...
          <p>API、AI 消息、工具调用和本机任务，按发生时间追踪。</p>
        </div>
      ...
      <div className="activity-filters">
        <div className="activity-search">
          ...
          <input
            aria-label="搜索系统日志"
            placeholder="搜索消息、路径、工具参数或任务 ID…"
            value={search}
      ...
      <div className="activity-overview" aria-label="活动时间轨道">
        <div className="activity-overview-meta">
          <span>
            活动轨道 <b>{page?.total.toLocaleString() ?? "—"}</b> 条匹配
          </span>
          <span>
            {events.length
      ...
      <div className={`activity-body ${selected ? "with-detail" : ""}`}>
        <div
      ...
      <footer className="activity-footer">
        <span>仅保存在本机 · 鉴权信息自动遮盖</span>
        <span>
          保留 {page?.retention_days ?? 30} 天 / 最近{" "}
          {(page?.max_records ?? 50000).toLocaleString()} 条 ·
          大型正文有截断标记 · 日志查询不重复记入
        </span>
      </footer>
    </section>
...
function loadingLabel(loading: boolean, count: number) {
  return loading ? "正在读取…" : `时间线 · 已载入 ${count.toLocaleString()} 条`;
}
