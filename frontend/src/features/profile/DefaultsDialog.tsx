...
import { builtinDefaults, builtinFields, defaultHasContent } from "./defaults";
        ...
        <header className="defaults-header">
          ...
          <div className="defaults-search">
            ...
            <input
              ...
              disabled={locked}
              onChange={
                /* 搜索只筛选导航，不改动编辑中的定义。 */ (event) => {
                  setTarget(null);
                  setQuery(event.target.value);
                }
              }
              onKeyDown={
                /* 回车跳到首个结果，中文输入法确认文字时不跳转。 */ (
                  event,
                ) => {
                  if (
                    event.key === "Enter" &&
