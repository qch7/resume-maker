        ...
        .map(
          /* 将每项自定义信息放在固定字段之后 */ (field, index) => (
            <div
              ...
              key={field.id}
              data-hidden={
                !(visibility?.[field.id] ?? field.visible) || undefined
              }
            >
              <input
                className="custom-field-name"
                aria-label={`${scope}自定义信息 ${index + indexOffset + 1} 名称`}
                title={field.label}
                placeholder="信息名称"
                value={field.label}
                maxLength={50}
                readOnly={
                  !editing || disabled || field.id.startsWith("default:")
                }
              ...
              <div className="field-control">
                <input
                  aria-label={`${scope}自定义信息 ${index + indexOffset + 1} 内容`}
