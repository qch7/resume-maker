...
项目 AI 请求使用 `source_access=provided-text-only`。本机 `source_context.py` 收集最多 200 个文件、单文件 128 KB、正文总计 350000 字符，并限制遍历数量，保留相对路径和行号；跳过敏感目录、凭据、二进制和链接。未提供内容不作为证据。统一隐私网关替换已知个人信息和常见身份格式后发送；模型仅能通过固定编号读取和搜索脱敏副本，没有任意文件、shell 或网络工具。返回引用在本机还原后继续由 `capture_evidence()` 核验原文件，未引用的任务不建立证据记录。
