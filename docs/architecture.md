...
模板导入在异步任务中处理已复制的原文件快照。`integrations/word/templates/prepare.py` 接受修订、解除绑定与锁定、清理失效注释引用；`integrations/word/templates/fields.py` 保留页码并冻结其他动态域的显示结果。`integrations/word/recovery.py` 优先保留原生 DOCX 结构，旧格式使用独立 Word 进程自动修复转换；可编辑 DOCX 保留 OOXML 资源和原生图形，尚不支持的对象报告具体问题；扫描页通过声明式 `RecoveredPage` 恢复可编辑段落与独立照片，有可靠文字层的 PDF 通过独立版面转换器恢复，再重新分配节点供 `TemplatePlan` 映射。页面恢复有一次自动重试，取消后不发布迟到内容；不可读的原文件明确失败，空白内容使用字段占位框架。独立图片模板通过本地 OCR 和脱敏整页图恢复文字、照片及装饰，扫描页保持原本地 OCR 入口，字体和复杂装饰需要人工核对；可编辑 DOCX 和可靠文字层 PDF 继续走本地结构解析。原生解析仍保留包资源上限，鉴权、项目引用、资料覆盖及映射完整性校验不变。识别每轮结构和资料覆盖通过后先执行本机 DOCX 试填，栏目容器或分页冲突参与下一轮映射修正；正式预览和导出共用通过检查的原生模板。`integrations/word/templates/anchors.py` 为借用正文段落的独立浮动绘图分出极小高度锚点，保留原坐标和组合关系；组合中的照片仅更换或删除其叶子图片，图形编号在外层绘图和组合子图形之间保持唯一，防止 Word 打开失败。
...
生产模板映射发送脱敏后的精确节点清单、OOXML 布局结构和带节点编号的马赛克拼图，不发送原始整页图。独立图片模板先通过脱敏整页通道恢复 DOCX，再进入映射。照片位置需人工核对；每轮带完整本地上下文并重新脱敏，最佳方案和原模板快照仍成对保留。本机试填及导出继续使用真实资料。
...
独立图片恢复通过 `PageImage` 将本机路径交给统一隐私出口，`providers/page_images.py` 本地 OCR 后使用同一个 `Redactor` 保护正文、页首身份区和低置信度行。文字原始像素全部移除，安全文字按坐标重绘，敏感行不透明覆盖；相邻图像色块合并后统一以最多八格打码，照片和复杂装饰不依赖人脸检测。全新 RGB PNG 及脱敏 OCR 坐标作为受控附件和文字证据交给模型。返回的每行文字和坐标须对应 OCR，隐私占位符只在本机还原，再校验几何并从本机原件裁剪照片、图标，重建可编辑 Word；后续映射继承本页学到的身份和凭据。字体及复杂图形可能因重绘损失细节，不能承诺完全匿名或精确复刻。扫描 PDF、扫描 Word 和普通图片参数继续走原 OCR 入口。
