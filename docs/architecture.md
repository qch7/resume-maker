...
| `infrastructure/migrations/` | 原始结构与增量迁移；版本 2 增加方案删除标记，版本 3 增加整体与子项目关联 |
...
1. **经历修订不可变**：发布或恢复创建新修订，`parent_id` 表示真实父版本；分支通过 `experience_branches.head_revision` 指向各自最新版本，`projects.head_revision` 保持指向主分支以兼容原有默认选择，不改旧版本正文。
2. **草稿带版本号**：按项目、基础修订和字段定位。保存或删除时检查版本，防止两个窗口静默覆盖。
3. **修改与提交分离**：编辑、增删和排序只写草稿；界面仅通过“提交为新版本”发布整个工作副本。内容未改变时确认草稿，不生成重复修订。底层单字段发布接口继续兼容旧调用，其余差异仍迁移到新修订对应的草稿中。
...
9. **整体与子项目相互独立**：多来源整体项目通过 `project_hierarchy` 关联单来源子项目；经历、草稿、快照、会话和简历引用仍按各自项目标识隔离。导入和来源更新在事务中同步子项目；升级时仅根据已登记路径补齐关系，不读取源码或调用模型。来源移除时解除分组并保留旧子项目历史。

经历历史由 `services/history.py` 管理，`revision_branches` 记录修订所属分支。创建分支时追加一个内容相同的起点修订，并可复制来源草稿到新修订的草稿空间；因此两个分支从同一版本出发也不会共享未发布修改。保存、恢复及 AI 建议采用均校验目标分支头，分支指针和修订、草稿迁移在一个 SQLite 写事务中完成。历史树按真实父子关系绘制，不按时间猜测；查看旧节点后必须创建分支或追加恢复才能继续发布。迁移 004 为旧项目建立 `main`，不重写旧版本或已有引用。经历数据不双写 Git，避免数据库与仓库出现部分提交。
