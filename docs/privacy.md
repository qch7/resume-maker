...
设置中的“隐私保护”可补充准确敏感值、在本机检测文字、查看和清除最近 10 份脱敏材料包。记录包括初始材料、输出契约、源码工具调用总次数及最近 20 次脱敏结果，**不是 CLI 最终 HTTP 请求的完整抓包**；审计保留数量不限制源码访问。`prepared` 只表示已准备，失败和取消也不能证明供应商未收到请求。
