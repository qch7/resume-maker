...
所有生产 AI 功能经过同一脱敏网关。原始资料留在本机，模型获得脱敏文字和布局信息；模板内嵌图片在本机打上马赛克后作为图片附件提供。结果还原后继续核验字段、模板引文和源码证据。当前实现保留 Windows 原生 Codex CLI，不需要 WSL、Docker、Windows Sandbox 或 GPU。
