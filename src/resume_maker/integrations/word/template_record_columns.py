...
from resume_maker.integrations.word.template_personal import paragraph_stream, slice_paragraph
...
def text_width(text, size):
    """保守估算中英文混排宽度，只用于分配列宽，实际换行仍由 Word 完成。"""
    return round(
        sum(
            1
...
def font_size(styles, paragraph):
    """读取运行和继承样式中的字号上界，避免日期标签比数值大时低估宽度。"""
...
def make_columns(paragraph, fragments, widths, left):
    """创建无边框固定列宽表格，不设行高或不换行约束，让长标题自然撑高一条记录。"""
...
def prepare_record_columns(styles, nodes, fields, records, source_nodes=None):
    """每份重复区使用全部记录的最大日期宽度，避免短名称、长名称各自生成不同列线。"""
