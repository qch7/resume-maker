...
from resume_maker.integrations.word.pdf_geometry import (
    BOX,
    SOURCE,
    WP,
    legacy_asset,
...
from resume_maker.integrations.word.pdf_header_items import (
    attach_header_assets,
    drawing_box,
    extract_items,
    header_region,
)
from resume_maker.integrations.word.template_contact_style import SLOT_VERSION
...
from resume_maker.integrations.word.template_record_columns import column_paragraph, text_width
...
def inline_drawing(anchor, width=None):
    """将已填入实际图片的绘图转换为行内对象，避免再次依赖旧段落的绝对偏移。"""
...
def source_style(items):
    """新增字段使用原 PDF 正文资料字体，不从无文字的图标运行继承 Word 默认字体。"""
    ordered = sorted(
        items,
        key=lambda item: (
