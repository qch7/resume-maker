...
from resume_maker.integrations.word.pdf_geometry import SOURCE, mark_paragraph


def text_counter(text):
    """按非空白字符统计原文覆盖，避免布局转换静默漏字或漏掉整个栏目。"""
...
def font_family(name):
    """去除 PDF 子集和字体样式后缀，避免把错误的子集族名当作系统字体。"""
...
def merge_bullet_table(block, column_left):
    """将误识别为两列表格的列表还原为文字块，避免多行正文与符号各自排版后错位。"""
...
def separate_blocks(blocks, column_left, *, split_lines=False):
    """大间隔、颜色变化和列表首行建立段落边界，避免标题与条目被合并后无法单独排序。"""
    from pdf2docx.text.TextBlock import TextBlock
    ...
    for block in blocks:
        ...
        for index, lines in enumerate(groups):
            ...
            # 原块的制表位相对旧起点；拆段后重算，避免左缩进与旧制表位叠加挤窄正文。
...
def capture_block(block, captured, column_left):
    """只包装当前解析块的输出，记录它生成的段落和源坐标，不猴子补丁全局类。"""
    ...
    def make(paragraph):
        """保留原字号的行距，并用悬挂缩进和制表符维持列表符号与正文间距。"""
        original(paragraph)
