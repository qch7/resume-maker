...
from resume_maker.integrations.word.template_map import paragraph_text, paragraph_texts, quote_range
...
def explicit_slot(node, nodes, keep):
    """带标签、图标或固定装饰的空位属于原版设计，不能当作任意留白重新安置。"""
...
def labelled_slot(donor, binding, label):
    """沿用相邻正文的缩进、行距和字体建立带标签段落，不复制图形、制表位或分节。"""
...
def prepare_project_slots(nodes, fields, values, keep):
    ...
    for field in sorted(fields, key=lambda field: ORDER.get(field.target, len(ORDER))):
        ...
        # 前面的补位已经改变段落顺序，不能再用源节点编号决定本次插入位置。
