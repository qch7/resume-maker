...
3. **独立会话**：左侧按项目排列会话，支持新建、直接切换、改名和归档；设置中可恢复归档会话。每条会话有独立历史、输入草稿和 Codex 会话 ID。
4. **编辑经历**：采用 AI 建议会先进入草稿。每条亮点的标题和正文均可人工编辑、单独保存；保存一条生成新的经历版本，其他未发布修改仍保留为草稿。可新增、删除、排序并恢复历史版本。
