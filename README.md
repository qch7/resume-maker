...
模型仅接收经过本机隐私网关处理的文字，不再启动具有文件或工具访问能力的 CLI 模型会话。个人资料、荣誉获奖人、证书编号、自定义敏感词及常见联系方式等替换为单轮占位符，返回结果在本机还原；凭据直接移除。旧模型会话、原始图片、照片和扫描件不会外发。

在“工作台设置 → 隐私保护”补充学校、单位等敏感词，使用本地检测查看替换效果，并检查最近 10 条实际请求。自动规则可能漏掉未知姓名、特殊格式和间接身份线索；详见 [隐私边界和兼容性](docs/privacy.md)。原始资料及 ZIP 备份仍由本机保存，旧版本已经发送的数据无法撤回。
...
| 含 API 凭据的 Codex 供应商配置（Responses 协议） | 通过无工具 API 进行项目分析、证书文字提取和模板适配；仅订阅登录暂不支持 |
...
4. **选荣誉**：上传证书保存在本机，可提取的 PDF 文字经脱敏后识别；图片和扫描件手动录入，核对后加入简历。
...
导入可编辑 Word 或有可靠文字层的 PDF 后，AI 根据脱敏文字和布局结构识别填写位置，再在本机用真实资料试填；照片位置需人工核对。图片、扫描件及需要视觉恢复的页面默认拦截外发，可使用内置模板或手动录入。
...
AI 功能从当前 CLI 配置读取供应商 API 凭据，设置中的“测试实际连接”可检查无工具 Responses 连接；项目源码由本机收集并脱敏，完成后留存引用文件和证据。
