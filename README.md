...
Resume Maker 是运行在本机的项目经历工作台。它通过 Codex CLI 直接只读关联目录中的当前源码，将 AI 建议交给用户确认；经历以不可变版本保存，每份简历明确引用具体版本，避免修改一处影响所有简历。

## 功能

- **项目与来源**：扫描项目集合，支持一个项目关联多个仓库，AI 按需读取当前文件，完成后留存引用原文、Git 状态和引文证据。
- **经历版本**：逐条或整段编辑、草稿恢复、冲突检查、历史恢复，AI 建议采用后先进入草稿。
- **独立会话**：每条会话独立保存历史、输入和模型会话标识；任务排队，支持取消和超时。
- **简历组合**：选择固定版本及亮点，拖动排序，复用不同模板和组合方案。
- **荣誉证书库**：独立管理荣誉，批量上传 PDF 或图片，AI 提取名称、奖项、单位、日期、获奖人和编号；对照原件核对修改、分类搜索，将已确认条目加入不同简历。原件与资料随 ZIP 备份恢复。
- **个人资料与栏目**：在项目经历、个人信息、栏目编排之间切换；填写照片、联系方式、成绩、教育、课程、证书与技能，自定义大栏目和子栏目，调整顺序与显隐。
- **本机路径选择**：模板、项目目录与 Codex 可执行文件均可通过 Windows 原生窗口选择，支持手动填写和追加多个来源文件夹。
- **AI 模板适配**：顶部独立“Word 模板”板块分析陌生 DOCX，可视化呈现姓名、联系方式、照片和重复栏目，支持点击原文人工调整、真实试填及模板重开另存。
- **文档导出**：支持内置完整简历和 AI 识别的完整模板；可下载 Word、追溯清单及可用的 PDF/分页预览。
...
Codex 复用当前用户的 CLI 配置，包括自定义 Provider；设置中的实际连接测试用于确认连通性。项目分析直接读取关联目录，程序不再提前复制整份源码或限制采集文件数量与大小；分析内容会交给你配置的 Provider 处理。
...
src/resume_maker/
  ...
  integrations/        # 来源与引用证据、Provider、OOXML 和 Word 适配
  cli.py               # 本机命令行入口
frontend/src/
  app/                 # 跨业务协调和工作台布局
...
完整模板适配支持 DOCX 正文、表格、文本框、页眉页脚和内嵌照片，识别结果支持人工核对和调整。图片形式的简历和不支持的 Word 对象需先整理后导入。Word 渲染不可用时，DOCX 仍可下载，页面会显示原因，不估算页数。
