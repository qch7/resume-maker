...
导入可编辑 Word 或有可靠文字层的 PDF 后，AI 根据脱敏文字和布局结构识别填写位置，再在本机用真实资料试填；照片位置需人工核对。图片和扫描件先用本地 OCR 恢复文字及位置，字体、照片和装饰需人工核对；原图不外发。
...
AI 功能使用原生 CLI 0.154.0，复用文件登录和供应商配置；“测试实际连接”会调用所选供应商。项目源码在本机收集并脱敏，CLI 通过只读材料工具分析，完成后留存引用文件和证据。
