...
- **沿用已有模板**：导入 Word、PDF 或图片，识别填写位置并试填，也可以直接使用内置模板。
...
| 已配置的 Codex CLI | 项目分析、证书识别和模板适配等 AI 功能 |
...
4. **选荣誉**：在“荣誉证书”中上传证书或手动录入，核对后将需要的条目加入当前简历。
...
导入 Word、PDF 或图片后，AI 识别姓名、联系方式、照片和各栏目的填写位置，再用当前资料生成试填预览；填错或漏填时可以选择对应内容修正。
...
AI 功能复用当前用户的 Codex CLI 配置，包括自定义 Provider，设置中的“测试实际连接”可检查连通性；项目分析直接只读关联目录，完成后留存引用文件和证据。
