...
def test_fork_drafts_save_restore_and_pinned_resume(catalog, project, populated):
    ...
    saved = catalog.save_revision(p, left["head_revision"], left["head_revision"])
    ...
    catalog.put_draft(p, saved["id"], "meta", {"title": "Pending title"}, 0)
    with pytest.raises(Problem, match="未保存草稿"):
        catalog.restore(p, left["head_revision"], saved["id"])
    complete = catalog.save_revision(p, saved["id"], saved["id"])
